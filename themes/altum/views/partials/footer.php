...
<div class="d-flex flex-column flex-lg-row justify-content-between mb-3 align-items-lg-center">
    <div class="d-flex flex-column align-items-start mb-3 mb-lg-0">
        ...
        <div class="d-none d-lg-block mt-3">
            ...
            <address>
                ...
                333 Peradeniya Rd, Kandy 20000, Sri Lanka.<br>
                <strong> Tel:</strong><br>
                +94 76 014 5559<br>
                <strong> E-mail:</strong><br>
                support@digitalstrategists.co<br>
            </address>
        </div>
    </div>
    ...
    <div class="col-12 col-lg-auto">
        ...
        <?php foreach(require APP_PATH . 'includes/admin_socials.php' as $key => $value): ?>
                <?php if(isset(settings()->socials->{$key}) && !empty(settings()->socials->{$key})): ?>
                    <a href="<?= sprintf($value['format'], settings()->socials->{$key}) ?>" class="mr-2 mr-lg-0 ml-lg-2 mb-2" target="_blank" data-toggle="tooltip" title="<?= $value['name'] ?>">
    <?php if (strpos(trim($value['icon']), '<svg') === 0): ?>
        <span style="display: inline-block; transform: translateY(-2px);"><?= $value['icon'] ?></span> <!-- Apply margin-top to the span containing the SVG -->
    <?php else: ?>
        <i class="<?= $value['icon'] ?> fa-fw fa-lg"></i> <!-- If it's not an SVG, use the regular font icon class -->
    <?php endif; ?>
</a>
                <?php endif ?>
            <?php endforeach ?>
